...
module variables
    ...
    integer,parameter                        :: sizex=96,sizey=96,sizez=96,zprocs=2,yprocs=2
...
subroutine data_in()   
    ...
    Mach = pi
    prandtl = 0.7d0
    Re = 574d0 !can be changed to match to match a paricular initial Re_lambda in isoturb
    Lewis = 1d0
    !*************************************
    ...
    vis_star = 0.1d0*exp((At(1)*log(T_star) + Bt(1))*log(T_star) + Ct(1))
...
subroutine stats()   
    ...
    do k=startz,endz
        do j=starty,endy
            do i=startx,endx
                ...
                vortx(i,j,k) = d6dy(w,i,j,k) - d6dz(v,i,j,k)
                vorty(i,j,k) = d6dz(u,i,j,k) - d6dx(w,i,j,k)
                vortz(i,j,k) = d6dx(v,i,j,k) - d6dy(u,i,j,k)
                vort(i,j,k) = sqrt(vortx(i,j,k)**2 + vorty(i,j,k)**2 + vortz(i,j,k)**2)
                div(i,j,k) = d6dx(u,i,j,k) + d6dy(v,i,j,k) + d6dz(w,i,j,k)
            end do
        end do
    end do

    vort_mag_bar = global_average(vort,1)
    vortx_bar = global_average(vortx,1)
    vorty_bar = global_average(vorty,1)
    vortz_bar = global_average(vortz,1)
    vort_bar = sqrt(vortx_bar**2 + vorty_bar**2 + vortz_bar**2)
    div_bar = global_average(div,1)
    
    vortx_p(:,:,:) = vortx(:,:,:) - vortx_bar
    vorty_p(:,:,:) = vorty(:,:,:) - vorty_bar
    vortz_p(:,:,:) = vortz(:,:,:) - vortz_bar
    div_p(:,:,:) = div(:,:,:) - div_bar
    ...
    vort_p_sq_bar = vortx_p_sq_bar + vorty_p_sq_bar + vortz_p_sq_bar
    div_p_sq_bar = global_average(div_p,2)
    ...
    Re_lambda = lambda*sqrt(turb_KE)*ro_bar/vis_bar     !Re*(lambda/lo_star)*(ro_bar/ro_star)*M_t/(vis_bar/vis_star)/sqrt(3d0)    !(both these expressions are equivalent)
    ! Re_lambda = lambda*sqrt(u_p_sq_bar)*ro_bar/vis_bar

    if (iter==0) then
        t_turb = lambda/sqrt(turb_KE/3d0)
        write(*,*)'turbulent time scale (non-dim)',t_turb/t_scale
...
double precision function global_average(var,power)
...
subroutine source(ws)
    ...
    double precision, dimension(2)                          :: k_eq,k_f,k_b
    double precision, dimension(n_sp)                       :: ro_s
    double precision                                        :: Z_s,R1,R2,R3,R4,R5
    double precision                                        :: sum
    integer                                                 :: i,j,k,m

    write(*,*)'source() is called'
    ws = 0d0

    ! do k=startz,endz
    !     do j=starty,endy
    !         do i=startx,endx
    !             ! if(T(i,j,k)>(T_star+50d0)) then
    !                 k_f(:) = cf(:)*T(i,j,k)**eta(:)*exp(-theta(:)/T(i,j,k))
    !                 Z_s = 10000d0/T(i,j,k)
    !                 k_eq(:) = cm(:)*exp(a1(:)/Z_s + a2(:) + a3(:)*log(Z_s) + a4(:)*Z_s + a5(:)*Z_s**2)
    !                 k_b(:) = (cf(:)*T(i,j,k)**eta(:)*exp(-theta(:)/T(i,j,k)))/k_eq(:)
    ...
    !                 do m = 1,n_sp
    !                         !Martin
    !                     ! R1 = R1 + k_b(m)*(ro_s(2)/Mw(2))*(ro_s(m)/Mw(m)) - k_f(m)*(ro_s(1)/Mw(1))*(ro_s(m)/Mw(m))
    !                         !Nitrogen Dissociation and Oxygen Dissociation
    !                     R1 = R1 + k_b(m)*(ro_s(2)/Mw(2))*(ro_s(2)/Mw(2))*(ro_s(m)/Mw(m)) - k_f(m)*(ro_s(1)/Mw(1))*(ro_s(m)/Mw(m))
    !                 end do

    !                 ws(i,j,k,1) = Mw(1)*R1
    !                 ws(i,j,k,2) = -ws(i,j,k,1)
    !             ! else
    !             !     ws(i,j,k,1) = 0d0
    !             !     ws(i,j,k,2) = 0d0
    !             ! end if
